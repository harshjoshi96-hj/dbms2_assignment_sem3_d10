...
BEGIN
    ...
    ELSE
        ...
        WHILE divisor * divisor <= last_fib LOOP
            IF MOD(last_fib, divisor) = 0 THEN
                is_prime := FALSE;
                EXIT;
            END IF;
            divisor := divisor + 1;
    ...
    WHILE num <= 100 LOOP
        ...
        WHILE divisor * divisor <= num LOOP
            IF MOD(num, divisor) = 0 THEN is_prime := FALSE; EXIT; END IF;
            divisor := divisor + 1;
        END LOOP;
        IF is_prime THEN
            DBMS_OUTPUT.PUT(num || ' ');
    ...
    d := 1;
    WHILE d < num LOOP
        IF MOD(num,d) = 0 THEN divisor_sum := divisor_sum + d; END IF;
        d := d + 1;
    END LOOP;
    IF divisor_sum = num THEN DBMS_OUTPUT.PUT_LINE(num || ' is PERFECT');
    ELSE DBMS_OUTPUT.PUT_LINE(num || ' is NOT PERFECT'); END IF;
END;
/